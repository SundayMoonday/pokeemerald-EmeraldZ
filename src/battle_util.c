                                       ...
                                       u32 holdEffectAtk, u32 holdEffectDef, u32 abilityAtk, u32 abilityDef)
{
	...
	DAMAGE_APPLY_MODIFIER(GetCloseCallModifier(damageCalcData->isClose));
...
void GetBattlerReacts(u32 battler1, u32 battler2)
{
...
{	
    if (GetBattlerHoldEffect(gBattlerAttacker, TRUE) == HOLD_EFFECT_LOADED_DICE){
		...
		GetBattlerReacts(gBattlerAttacker, gBattlerTarget);
	}
...
void ClearDamageCalcResults(void)
{
    for (u32 battler = 0; battler < MAX_BATTLERS_COUNT; battler++)
    {
		...
		gSpecialStatuses[battler].closeCall = FALSE;
    }

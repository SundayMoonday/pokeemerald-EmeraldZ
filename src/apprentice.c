    ...
    while (i < 5)
    {
        if (Random() % 2 == 0 || needTMs == TRUE)
        {
            ...
            do
            {
                ...
                do
                {
                    id = Random() % (NUM_TECHNICAL_MACHINES + NUM_TECHNICAL_RECORDS + NUM_HIDDEN_MACHINES);
                    shouldUseMove = CanLearnTeachableMove(species, ItemIdToBattleMoveId(ITEM_TR01 + id));
                }
                while (!shouldUseMove);

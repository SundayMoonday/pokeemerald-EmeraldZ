...
u32 GetBattlerTotalAwareStatArgs(u32 battler, u32 ability, u32 holdEffect)
{
	u32 aware = gBattleMons[battler].awareness;
    ...
    if (ability == ABILITY_INNER_FOCUS || ability == ABILITY_KEEN_EYE || ability == ABILITY_ANTICIPATION || ability == ABILITY_ANALYTIC || ability == ABILITY_MINDS_EYE){
		...
		aware = (GetHighestStatId(battler) == STAT_AWARE) ? (aware * 150) / 100 : aware;
	}
    ...
    // stat stages
	aware *= gStatStageRatios[gBattleMons[battler].statStages[STAT_AWARE]][0];
    aware /= gStatStageRatios[gBattleMons[battler].statStages[STAT_AWARE]][1];
...
u32 GetBattlerTotalAwareStat(u32 battler)
{
    u32 ability = GetBattlerAbility(battler);
    u32 holdEffect = GetBattlerHoldEffect(battler, TRUE);
    return GetBattlerTotalAwareStatArgs(battler, ability, holdEffect);
}

u32 GetBattlerTotalReactStatArgs2(u32 battler, u32 ability, u32 holdEffect)
{
    ...
    if (ability == ABILITY_QUICK_FEET && gBattleMons[battler].status1 & STATUS1_ANY){
		...
		react = (GetHighestStatId(battler) == STAT_REACT) ? (react * 150) / 100 : react;
	}
    else if (ability == ABILITY_QUARK_DRIVE && !(gBattleMons[battler].status2 & STATUS2_TRANSFORMED) && (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN || gDisableStructs[battler].boosterEnergyActivates)){
		react = (GetHighestStatId(battler) == STAT_REACT) ? (react * 150) / 100 : react;
	}
...
u32 GetBattlerTotalReactStat2(u32 battler)
{
    u32 ability = GetBattlerAbility(battler);
    u32 holdEffect = GetBattlerHoldEffect(battler, TRUE);
    return GetBattlerTotalReactStatArgs2(battler, ability, holdEffect);
}
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
	...
	u32 battlerAtkReact = GetBattlerTotalReactStat2(battlerAtk);
    ...
    if (HasWeatherEffect() && gBattleWeather & B_WEATHER_FOG)
	 ...
	 if (battlerDefAware > battlerAtkReact && IsBattleMoveStatus(move)){
...
u32 CalcCritAware(u32 battlerAtk, u32 battlerDef)
{
	...
	if (battlerAtkAware > battlerDefAware){
		battlerAtkAware *= 2;
		critBonusRoll = ((battlerAtkAware * 20) / battlerDefAware) / 10;
	}
...
u32 CalcCloseAware(u32 battlerAtk, u32 battlerDef)
{
	...
	if (abilityDef == ABILITY_NO_GUARD || abilityAtk == ABILITY_NO_GUARD){
		critBonusRoll = 0;
	}else{
		if (battlerDefReact > battlerAtkAware){
			battlerDefReact *= 2;
			critBonusRoll = ((battlerDefReact * 10) / battlerAtkAware) / 10;
		} else {
			critBonusRoll = 0;
		}
	}
...
static void Cmd_critcalc(void)
{
    ...
    for (u32 battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        if (gBattleTypeFlags & (BATTLE_TYPE_WALLY_TUTORIAL | BATTLE_TYPE_FIRST_BATTLE)){
			...
			gSpecialStatuses[battlerDef].closeCall = FALSE;
		}
        else
        {
            if (B_CRIT_CHANCE == GEN_1)
            ...
            else
            {
				...
				if (gSpecialStatuses[battlerDef].criticalHit < 0){
					break;
		}
		}
            }
		...
		if (gSpecialStatuses[battlerDef].criticalHit == FALSE){
		// Check for Close Call	
		closeRoll = CalcCloseAware(gBattlerAttacker, gBattlerTarget);
		gSpecialStatuses[battlerDef].closeCall = RandomWeighted(RNG_CLOSE_CALL, 10, closeRoll);
	}
...
static void Cmd_damagecalc(void)
{
    ...
    if (IsSpreadMove(moveTarget))
    {
        ...
        for (battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
        {
			...
			damageCalcData.isClose = gSpecialStatuses[battlerDef].closeCall;
    ...
    else
    {
		...
		damageCalcData.isClose = gSpecialStatuses[gBattlerTarget].closeCall;
...
static void Cmd_closemessage(void)
{
    ...
    if (gBattleControllerExecFlags == 0)
    {
        if (gSpecialStatuses[gBattlerTarget].closeCall && !(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT))
        {
...
void SetMoveEffect(bool32 primary, bool32 certain)
{
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_AWARE_MINUS_1:
            ...
            case MOVE_EFFECT_AWARE_MINUS_2:
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (GetMonData(&gPlayerParty[*expMonId], MON_DATA_HP) && GetMonData(&gPlayerParty[*expMonId], MON_DATA_LEVEL) != MAX_LEVEL)
            {
                ...
                gBattleResources->beforeLvlUp->stats[STAT_AWARE] = GetMonData(&gPlayerParty[*expMonId], MON_DATA_AWARE);
...
static void Cmd_drawlvlupbox(void)
{
    ...
    if (gBattleScripting.drawlvlupboxState == 0)
    {
        ...
        if (IsMonGettingExpSentOut())
            ...
            gBattleScripting.drawlvlupboxState = 3; //Revert to 1 when GUI is fixed lol
    }
...
static u16 ReverseStatChangeMoveEffect(u16 moveEffect)
{
    ...
    case MOVE_EFFECT_AWARE_PLUS_1:
        return MOVE_EFFECT_AWARE_MINUS_1;
    case MOVE_EFFECT_ACC_PLUS_1:
    ...
    case MOVE_EFFECT_AWARE_MINUS_1:
        return MOVE_EFFECT_AWARE_PLUS_1;
    case MOVE_EFFECT_ACC_MINUS_1:
    ...
    case MOVE_EFFECT_AWARE_PLUS_2:
        return MOVE_EFFECT_AWARE_MINUS_2;
    case MOVE_EFFECT_ACC_PLUS_2:
    ...
    case MOVE_EFFECT_AWARE_MINUS_2:
        return MOVE_EFFECT_AWARE_PLUS_2;
    case MOVE_EFFECT_ACC_MINUS_2:
...
void BS_SwapStats(void)
{
    ...
    case STAT_SPDEF:
        ...
        break;
	case STAT_REACT:
        SWAP(gBattleMons[gBattlerAttacker].reaction, gBattleMons[gBattlerTarget].reaction, temp);
        break;
    case STAT_AWARE:
        SWAP(gBattleMons[gBattlerAttacker].awareness, gBattleMons[gBattlerTarget].awareness, temp);
        break;
    }

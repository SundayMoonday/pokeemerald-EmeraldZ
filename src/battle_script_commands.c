...
u32 GetBattlerTotalAwareStatArgs(u32 battler, u32 ability, u32 holdEffect)
{
    ...
    if (ability == ABILITY_INNER_FOCUS || ability == ABILITY_KEEN_EYE || ability == ABILITY_ANTICIPATION || ability == ABILITY_ANALYTIC || ability == ABILITY_MINDS_EYE)
        ...
        aware = (aware * 75) / 100;
	else if (ability == ABILITY_PROTOSYNTHESIS && gBattleWeather & B_WEATHER_SUN && highestStat == STAT_AWARE)
        aware = (aware * 150) / 100;
	 else if (ability == ABILITY_QUARK_DRIVE && gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN && highestStat == STAT_AWARE)
        aware = (aware * 150) / 100;
...
static void Cmd_critcalc(void)
{
	...
	gIsCloseCall = FALSE;
        ...
        gIsCriticalHit = RandomWeighted(RNG_CRITICAL_HIT, sGen7CriticalHitOdds[critChance] - 1, 1);
		if (gIsCriticalHit < 1){
			break;
		}
		}
...
static void Cmd_damagecalc(void)
{
	...
	damageCalcData.isClose = gIsCloseCall;

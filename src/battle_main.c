...
EWRAM_DATA u8 gIsCloseCall = FALSE;
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            if (partyData[i].ev != NULL)
            {
                ...
                SetMonData(&party[i], MON_DATA_AWARE_EV, &(partyData[i].ev[7]));
            }
...
u32 GetBattlerTotalSpeedStatArgs(u32 battler, u32 ability, u32 holdEffect)
{
    ...
    else if (ability == ABILITY_PROTOSYNTHESIS && gBattleWeather & B_WEATHER_SUN){
    ...
    else if (ability == ABILITY_QUARK_DRIVE && gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN){
        speed = (GetHighestStatId(battler) == STAT_SPEED) ? (speed * 150) / 100 : speed;
	}
	 else if (ability == ABILITY_QUARK_DRIVE && gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN){
        react = (GetHighestStatId(battler) == STAT_REACT) ? (react * 150) / 100 : react;
    ...
    if (holdEffect == HOLD_EFFECT_MACHO_BRACE || holdEffect == HOLD_EFFECT_POWER_ITEM)
        ...
        speed = (speed * 150) / 100;
    else if (holdEffect == HOLD_EFFECT_QUICK_POWDER && gBattleMons[battler].species)
        speed = (speed * 150) / 100;
